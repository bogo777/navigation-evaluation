...
public class HealthVialsBot extends UT2004BotModuleController<UT2004Bot> {
	...
	public void botInitialized(GameInfo gameInfo, ConfigChange currentConfig, InitedMessage init) {
		...
		pathExecutor.addStuckDetector(new UT2004TimeStuckDetector(bot, 3000, 10000)); // if the bot does not move for 3 seconds, considered that it has stuck
	...
	public void logic() {
		...
		if (visible || spawned) {
			pathExecutor.followPath(pathPlanner.computePath(bot.getLocation(), target));			
		} else {
