...
public class FloydWarshallMap extends SensorModule<IGhostAgent> implements IPathPlanner<NavPoint> {
	...
	public void refreshPathMatrix() {
		synchronized(mutex) {
			...
			if (log.isLoggable(Level.WARNING)) log.warning(this + ": Path matrix refreshed!");
		}		
	}
		...
		for (int i = 0; i < size; i++) {
			for (int j = 0; j < size; j++) {
				if (pathMatrix[i][j].getDistance() == Float.POSITIVE_INFINITY) {
					if (log.isLoggable(Level.FINER)) log.finer("Unreachable path from " + navPoints.get(i).getId().getStringId()
