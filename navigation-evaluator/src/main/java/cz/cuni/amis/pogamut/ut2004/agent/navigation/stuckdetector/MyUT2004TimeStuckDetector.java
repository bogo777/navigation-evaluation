 ...
 */
public class MyUT2004TimeStuckDetector implements IStuckDetector {
    ...
    public MyUT2004TimeStuckDetector(UT2004Bot bot) {
        this(bot, DEFAULT_TIMEOUT, DEFAULT_WAITING_TIMEOUT);
    }

    public MyUT2004TimeStuckDetector(UT2004Bot bot, double timeoutMillis,
        ...
        if (botWaiting) {
            if (currentTime - lastMovementTime >= waitingTimeout) {
                ...
                if (log != null && log.isLoggable(Level.WARNING)) {
                    log.warning("Bot is WAITING for more than " + waitingTimeout + " ms, considering that it has stuck.");
                }
            }
        } else {
            if (currentTime - lastMovementTime >= timeout) {
                stuck = true;
                if (log != null && log.isLoggable(Level.WARNING)) {
                    log.warning("Bot should be moving but it is standing still for more than " + timeout + " ms, considering that it has stuck.");
                }
            }
        }
    }
