...
public class MyUT2004PositionStuckDetector implements IStuckDetector {
	...
	public MyUT2004PositionStuckDetector(UT2004Bot bot) {
		this(bot, DEFAULT_HISTORY_LENGTH, DEFAULT_MIN_DIAMETER, DEFAULT_MIN_Z);
	}
	
	public MyUT2004PositionStuckDetector(UT2004Bot bot, int historyLength, double minMovementDiameter, double minMovementZ) {
		if (this.log == null) {
	...
	public void eventEndMessage(EndMessage event) {
		...
		if (locationHistory.size() == historyLength) {
			...
			if (maxDistance < this.minMovementDiameter && maxHeight < this.minMovementZ) {
				if (log != null && log.isLoggable(Level.WARNING)) log.warning("Bot stuck detected. Distance: " + maxDistance + " < " + this.minMovementDiameter + " && Height: " + maxHeight + " < " + minMovementZ);
				stuck = true;
			} else {
				stuck = false;
			}
