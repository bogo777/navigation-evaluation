...
public class NavMeshNavigator <PATH_ELEMENT extends ILocated> extends AbstractUT2004PathNavigator<PATH_ELEMENT>
{
     ...
     */
    private NavMeshNavigator.Stage navigStage = NavMeshNavigator.Stage.COMPLETED;

    /**
     * Current focus of the bot, if null, provide default focus.
     * <p><p>
     * Filled at the beginning of the {@link NavMeshNavigator#navigate(ILocated, int)}.
     */
	...
	/** {@link Self} listener */
	private NavMeshNavigator.SelfListener selfListener;
	...
	public void reset() {
		...
		navigStage = NavMeshNavigator.Stage.COMPLETED;
		setBotWaiting(false);
    ...
    protected void initDirectNavigation (Location dest)
    {
        ...
        if (log != null && log.isLoggable(Level.FINE)) log.fine (
            "NavMeshNavigator.initDirectNavigation(): initializing direct navigation"
            + ", distance " + distance
        );
    ...
    protected void initPathNavigation(Location dest, List<PATH_ELEMENT> path)
    {
        ...
        if (log != null && log.isLoggable(Level.FINE)) 
        	log.fine (
        			"NavMeshNavigator.initPathNavigation(): initializing path navigation"
        			+ ", nodes " + path.size ()
        	);
    ...
    protected NavMeshNavigator.Stage keepNavigating ()
    {
        ...
        if (log != null && log.isLoggable(Level.FINE)) {
        	if (navigLastNode != null) {
        		log.fine("NavMeshNavigator.keepNavigating(): navigating from " + navigLastNode.getId().getStringId() + navigLastNode.getLocation() );    	
        	} else
        	if (navigLastLocation != null) {
        		log.fine("NavMeshNavigator.keepNavigating(): navigating from " + navigLastLocation + " (navpoint is unknown)" );
        	}
        	if (navigCurrentNode != null) {
        		log.fine("NavMeshNavigator.keepNavigating(): navigating to   " + navigCurrentNode.getId().getStringId() + navigCurrentNode.getLocation() );
        	} else
        	if (navigCurrentLocation != null) {
        		log.fine("NavMeshNavigator.keepNavigating(): navigating to       " + navigCurrentLocation + " (navpoint is unknown)" );
        	}
        	if (navigLastLocation != null && navigCurrentLocation != null) {
        		log.fine("NavMeshNavigator.keepNavigating(): distance in-between " + navigCurrentLocation.getDistance(navigLastLocation));
        	}
        }
     ...
     */
    private NavMeshNavigator.Stage initDirectly(Location dest)
    {
        ...
        return navigStage = NavMeshNavigator.Stage.REACHING;
    }
     ...
     * @return Next stage of the navigation progress.
     */
    private NavMeshNavigator.Stage navigDirectly ()
    {    	
        ...
        {
            if (log != null && log.isLoggable(Level.FINE)) log.fine ("NavMeshNavigator.navigDirectly(): destination close enough: " + distance);
            return NavMeshNavigator.Stage.COMPLETED;
        }
        ...
        {
            if (log != null && log.isLoggable(Level.FINE)) log.fine ("NavMeshNavigator.navigDirectly(): direct navigation failed");
            return NavMeshNavigator.Stage.CRASHED;
        }

        // well, we're still running
        if (log != null && log.isLoggable(Level.FINEST)) log.finer ("NavMeshNavigator.navigDirectly(): traveling directly, distance = " + distance);
        return navigStage;
    }
    ...
    /**
     * How many path elements we have iterated over before selecting the current {@link NavMeshNavigator#navigNextLocation}.
     */
    ...
    /**
     * If {@link NavMeshNavigator#navigLastLocation} is a {@link NavPoint} or has NavPoint near by, its instance
    ...
    /**
     * If {@link NavMeshNavigator#navigCurrentLocation} is a {@link NavPoint} or has NavPoint near by,
    ...
    /**
     * If {@link NavMeshNavigator#navigNextLocation} is a {@link NavPoint} or has NavPoint near by,
     * its instance is written here (null otherwise).
     */
    private NavPoint navigNextNode = null;
    
    /**
     * Returns {@link NavPoint} instance for a given location. If there is no navpoint in the vicinity of {@link NavMeshNavigator#CLOSE_ENOUGH}
     * null is returned.
     * 
     * @param location
     * @return
    ...
    private boolean initAlongPath(Location dest, List<PATH_ELEMENT> path)
    {
        ...
        prepareNextNode();
        // reset navigation stage
        navigStage = NavMeshNavigator.Stage.NAVIGATING;
     ...
     * @return Next stage of the navigation progress.
     */
    private NavMeshNavigator.Stage navigAlongPath()
    {
     ...
     * <p><p>
     * If necessary just recalls {@link NavMeshNavigator#prepareNextNodeTeleporter()}.
     */
    ...
    {    	
        ...
        if (navigCurrentNode == null) {
        	...
        	if (log != null && log.isLoggable(Level.FINE)) 
        		log.fine (
                    "NavMeshNavigator.switchToNextNode(): switch to next location " + navigCurrentLocation
        	...
        	if (navigCurrentNode.isTeleporter()) {
        		navigStage = NavMeshNavigator.Stage.TeleporterStage();
        	} else
	        // is this next node a mover?
	        if (navigCurrentNode.isLiftCenter())
	        {
	            // setup mover sequence
	            navigStage = NavMeshNavigator.Stage.FirstMoverStage();	            
	            resetNavigMoverVariables();
	        ...
	        if (log != null && log.isLoggable(Level.FINE)) 
	        	log.fine (
		            "NavMeshNavigator.switchToNextNode(): switch to next node " + navigCurrentNode.getId().getStringId()
		            + ", distance " + localDistance
     ...
     * @return Next stage of the navigation progress.
     */
    private NavMeshNavigator.Stage navigToCurrentNode (boolean useFocus)
    {
    	if (navigCurrentNode != null) {
        ...
        if (!runner.runToLocation (navigLastLocation, firstLocation, secondLocation, focus, navigCurrentLink, (navigCurrentNode == null ? true : isReachable(navigCurrentNode)))) {
            if (log != null && log.isLoggable(Level.FINE)) log.fine ("NavMeshNavigator.navigToCurrentNode(): navigation to current node failed");
            return NavMeshNavigator.Stage.CRASHED;
        }

        // we're still running
        if (log != null && log.isLoggable(Level.FINEST)) log.finest ("NavMeshNavigator.navigToCurrentNode(): traveling to current node, distance = " + localDistance);
     ...
     * @return Next stage of the navigation progress.
     */
    private NavMeshNavigator.Stage navigThroughMover ()
    {
    	NavMeshNavigator.Stage stage = navigStage;
    	        
        if (navigCurrentNode == null) {
        	if (log != null && log.isLoggable(Level.WARNING)) log.warning("NavMeshNavigator.navigThroughMover("+stage+"): can't navigate through the mover without the navpoint instance (navigCurrentNode == null)");
        	return NavMeshNavigator.Stage.CRASHED;
        }
        
        Mover mover = (Mover) bot.getWorldView().get(navigCurrentNode.getMover());
        if (mover == null) {
        	if (log != null && log.isLoggable(Level.WARNING)) log.warning("NavMeshNavigator.navigThroughMover("+stage+"): can't navigate through the mover as current node does not represent a mover (moverId == null): " + navigCurrentNode);
        	return NavMeshNavigator.Stage.CRASHED;
        }
    	...
    	if (navigStage == NavMeshNavigator.Stage.AWAITING_MOVER) {   
    		// Aren't we under the mover?
    		if (zDistance > 0 && moverRidingUp) {
    			// we're under the mover and the mover is riding up...
    			if (log != null && log.isLoggable(Level.FINER)) {
	            	log.finer (
		                "NavMeshNavigator.navigThroughMover("+stage+"): we are UNDER the mover and mover is RIDING UP ... getting back to waiting position"
	            ...
	            {
	                if (log != null && log.isLoggable(Level.FINE)) log.fine ("NavMeshNavigator.navigThroughMover("+stage+"): navigation to wait-for-mover node failed");
	                return NavMeshNavigator.Stage.CRASHED;
	            }
    			return navigStage;
    		}
    		...
    		if (hDistance > 400) {
    			if (log != null && log.isLoggable(Level.WARNING)) log.warning("NavMeshNavigator.navigThroughMover("+stage+"): failed to get onto the mover as its 2D distance is > 400, hDistance " + hDistance + ", unsupported!");
                return NavMeshNavigator.Stage.CRASHED;
    		}
	        ...
	        if (zDistance > 30 && moverRidingUp) // mover is riding UP and is already above us, we won't make it...
	        {
	        	...
	        	if (log != null && log.isLoggable(Level.FINER)) {
	            	log.finer (
		                "NavMeshNavigator.navigThroughMover("+stage+"): waiting for the mover to come"
	            ...
	            if (!runner.runToLocation(memory.getLocation(), navigLastLocation, null, navigCurrentLocation, navigCurrentLink, (navigLastNode == null ? true : isReachable(navigLastNode))))  {
	                if (log != null && log.isLoggable(Level.FINE)) log.fine ("NavMeshNavigator.navigThroughMover("+stage+"): navigation to last node failed");
	                return NavMeshNavigator.Stage.CRASHED;
	            }
    	...
    	if (navigStage == NavMeshNavigator.Stage.RIDING_MOVER) {
    		...
    		if (navigMoverRideDownCount > 2 || navigMoverRideUpCount > 2) {
    			...
    			if (log != null && log.isLoggable(Level.FINE)) log.fine ("NavMeshNavigator.navigThroughMover("+stage+"): navigation to mover exit node failed, we've rided twice up & down and there was no place suitable to exit the mover in order to get to get to " + navigCurrentNode);
	            return NavMeshNavigator.Stage.CRASHED;
    		}
    		
    		if (hDistance > 400) {
    			if (log != null && log.isLoggable(Level.WARNING)) log.warning("NavMeshNavigator.navigThroughMover("+stage+"): navigation to mover exit node failed, the node is too far, hDistance " + hDistance + " > 400, unsupported (wiered navigation graph link)");
	            return NavMeshNavigator.Stage.CRASHED;
    		}
    		...
    		if ( Math.abs(zDistance) > 50 ) {
    			...
    			if (log != null && log.isLoggable(Level.FINER)) 
 	            	log.finer (
 		                "NavMeshNavigator.navigThroughMover("+stage+"): riding the mover"               
 		                + " | zDistance " + zDistance + ", hDistance " + hDistance + ", mover " + (moverRidingUp ? "riding UP" : (moverRidingDown ? "riding DOWN" : moverStandingStill ? "standing STILL" : " movement unknown"))
 		                + ", node " + navigCurrentNode.getId().getStringId()
 		            ); 	
     ...
     * @return Next stage of the navigation progress.
     */
    private NavMeshNavigator.Stage navigThroughTeleport()
    {
    	if (navigCurrentNode != null) {
        ...
        // we're still running
        if (log != null && log.isLoggable(Level.FINEST)) log.finest ("NavMeshNavigator.navigToCurrentNode(): traveling to current node");        
    ...
    public enum Stage
    {
        ...
        REACHING ()
        {
            protected NavMeshNavigator.Stage next () { return this; }
        },
        /**
         ...
         */
        AWAITING_MOVER (NavMeshNavigator.MoverStageType.WAITING)
        {
            protected NavMeshNavigator.Stage next () { return RIDING_MOVER; }
        },
        /**
         * Waiting for a mover to ferry.
         */
        RIDING_MOVER (NavMeshNavigator.MoverStageType.RIDING)
        {
            protected NavMeshNavigator.Stage next () { return NAVIGATING; }
        },
        /**
         * Navigation cancelled by outer force.
         */
        CANCELED (NavMeshNavigator.TerminatingStageType.FAILURE)
        {
            protected NavMeshNavigator.Stage next () { return this; }
        },
        /**
         ...
         */
        TIMEOUT (NavMeshNavigator.TerminatingStageType.FAILURE)
        {
            protected NavMeshNavigator.Stage next () { return this; }
        },
        /**
         ...
         */
        CRASHED (NavMeshNavigator.TerminatingStageType.FAILURE)
        {
            protected NavMeshNavigator.Stage next () { return this; }
        },
        /**
         ...
         */
        COMPLETED (NavMeshNavigator.TerminatingStageType.SUCCESS)
        {
            protected NavMeshNavigator.Stage next () { return this; }
        },
        /**
         ...
         */
        TELEPORT (NavMeshNavigator.TeleportStageType.GOING_THROUGH) {
        	protected NavMeshNavigator.Stage next() { return NAVIGATING; };
        };
        ...
        private Stage(NavMeshNavigator.TeleportStageType type) {
        	this.mover = false;
        	this.teleport = true;
        	this.failure = false;
        	this.terminated = false;
         ...
         */
        private Stage (NavMeshNavigator.MoverStageType type)
        {
            this.mover = true;
            this.teleport = false;
            this.terminated = false;
         ...
         */
        private Stage (NavMeshNavigator.TerminatingStageType type)
        {
            this.mover = false;
            this.teleport = false;
         ...
         */
        protected abstract NavMeshNavigator.Stage next ();
        ...
        protected static NavMeshNavigator.Stage TeleporterStage() {
        	return NavMeshNavigator.Stage.TELEPORT;
        }
    }
    ...
    public NavMeshNavigator (UT2004Bot bot, AgentInfo info, AdvancedLocomotion move, IUT2004PathRunner runner, Logger log)
    {
        ...
        this.selfListener = new NavMeshNavigator.SelfListener( bot.getWorldView() );
